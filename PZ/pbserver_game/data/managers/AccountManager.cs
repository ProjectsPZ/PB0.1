...
namespace Game.data.managers
{
  public static class AccountManager
  {
    ...
    public static void AddAccount(Account acc)
    {
      ...
      {
        if (AccountManager._contas.ContainsKey(acc.player_id))
          return;
        AccountManager._contas.Add(acc.player_id, acc);
      }
    }
    ...
    public static Account getAccount(long id, int searchFlag)
    {
      ...
      try
      {
        ...
        return AccountManager._contas.TryGetValue(id, out account) ? account : AccountManager.getAccountDB((object) id, 2, searchFlag);
      }
      catch
      {
        return (Account) null;
    ...
    public static Account getAccount(long id, bool noUseDB)
    {
      ...
      {
        Account account = (Account) null;
        return AccountManager._contas.TryGetValue(id, out account) ? account : (noUseDB ? (Account) null : AccountManager.getAccountDB((object) id, 2));
      }
      catch
      {
        return (Account) null;
    ...
    public static Account getAccount(string text, int type, int searchFlag)
    {
      if (string.IsNullOrEmpty(text))
        return (Account) null;
      IList<Account> values = AccountManager._contas.Values;
