  ...
  public class Room
  {
    ...
    public Room(int roomId, Channel ch)
    {
      ...
      for (int slotIdx = 0; slotIdx < this._slots.Length; ++slotIdx)
        this._slots[slotIdx] = new SLOT(slotIdx);
    ...
    public void LoadHitParts()
    {
      ...
      Logger.warning("By: " + (object) next + "/ Hits: " + BitConverter.ToString(array));
      this.HitParts = array;
    ...
    {
      this.UniqueRoomId = (uint) ((ConfigGS.serverId & (int) byte.MaxValue) << 20 | (this._channelId & (int) byte.MaxValue) << 12 | this._roomId & 4095);
    }
    ...
    {
      if (this.special != (byte) 6 && this.special != (byte) 8)
        return this.special == (byte) 9;
      return true;
    }
    ...
    public void StartBomb()
    {
      try
      {
        this.bomba.StartJob(42000, (TimerCallback) (callbackState =>
        {
    ...
    public void StartBattle(bool updateInfo)
    {
      ...
      using (BATTLE_READYBATTLE_PAK battleReadybattlePak = new BATTLE_READYBATTLE_PAK(this))
      {
        ...
        foreach (Account allPlayer in this.getAllPlayers(SLOT_STATE.READY, 0))
        {
          SLOT slot = this.getSlot(allPlayer._slotId);
          if (slot != null)
          {
            slot.withHost = true;
        ...
        for (int index = 0; index < 16; ++index)
        {
          ...
          if (!slot.check && slot.state == SLOT_STATE.BATTLE && this.getPlayerBySlot(slot, out player))
          {
            ...
            if (!isBotMode)
            {
              ...
              {
                slot.exp = (int) ((double) slot.Score + (double) num2 / 2.5 + (double) slot.allDeaths * 2.2 + (double) (slot.objetivos * 20));
                slot.gp = (int) ((double) slot.Score + (double) num2 / 3.0 + (double) slot.allDeaths * 2.2 + (double) (slot.objetivos * 20));
                slot.money = (int) ((double) (slot.Score / 2) + (double) num2 / 6.5 + (double) slot.allDeaths * 1.5 + (double) (slot.objetivos * 10));
              }
              else
              {
                slot.exp = (int) ((double) slot.Score + (double) num2 / 2.5 + (double) slot.allDeaths * 1.8 + (double) (slot.objetivos * 20));
                slot.gp = (int) ((double) slot.Score + (double) num2 / 3.0 + (double) slot.allDeaths * 1.8 + (double) (slot.objetivos * 20));
                slot.money = (int) ((double) (slot.Score / 2) + (double) num2 / 6.5 + (double) slot.allDeaths * 1.1 + (double) (slot.objetivos * 10));
              }
            ...
            {
              int num2 = (int) this.IngameAiLevel * (150 + slot.allDeaths);
              if (num2 == 0)
                ++num2;
              int num3 = slot.Score / num2;
              slot.gp += num3;
              slot.exp += num3;
            }
            ...
            {
              Logger.warning("[1]SLOT XP: " + (object) slot.exp + "; GOLD: " + (object) slot.gp + "; MONEY: " + (object) slot.money);
              Logger.warning("InBattleTime: " + (object) num1 + " [Finish: " + now.ToString("yyMMddHHmmss") + "; Start: " + slot.startTime.ToString("yyMMddHHmmss") + "]");
              Logger.warning("allDeaths: " + (object) slot.allDeaths + "; Objs: " + (object) slot.objetivos + "; IsBOT: " + isBotMode.ToString());
              slot.exp = 2;
              slot.gp = 2;
              slot.money = 2;
            }
            ...
            {
              percent1 += player.pc_cafe == 2 ? 60 : 30;
              percent2 += player.pc_cafe == 2 ? 50 : 20;
    ...
    private void CalculateSpecialCM(Clan clan, int winnerTeam, int teamIdx)
    {
      ...
      if (winnerTeam == teamIdx)
      {
        ...
        Logger.warning("Clan: " + (object) clan._id + "; Earned Points: " + (object) num);
        clan._pontos += num;
        Logger.warning("Clan: " + (object) clan._id + "; Final Points: " + (object) clan._pontos);
      }
      else if ((double) clan._pontos == 0.0)
      {
      ...
      else
      {
        ...
        Logger.warning("Clan: " + (object) clan._id + "; Losed Points: " + (object) num);
        clan._pontos -= num;
        Logger.warning("Clan: " + (object) clan._id + "; Final Points: " + (object) clan._pontos);
      }
    }
      ...
      foreach (int team in this.GetTeamArray(teamIdx))
      {
        ...
        if (slot._playerId == 0L && slot.state == SLOT_STATE.EMPTY)
        {
          ...
          if (p._slotId == this._leader)
            this._leader = team;
          Logger.LogProblems("[Room.SwitchNewSlot] Jogador '" + (object) p.player_id + "' '" + p.player_name + "'; OldSlot: " + (object) p._slotId + "; NewSlot: " + (object) team, "errorC");
          p._slotId = team;
    ...
    {
      SLOT slot1 = this._slots[newSlotId];
      SLOT slot2 = this._slots[oldSlotId];
      if (changeReady)
      {
    ...
    public void setNewLeader(int leader, int state, int oldLeader, bool updateInfo)
    {
      ...
      if (this._leader != -1)
      {
        SLOT slot = this._slots[this._leader];
    ...
    private void BaseRemovePlayer(Account player, SLOT slot, bool WarnAllPlayers, int quitMotive)
    {
      ...
      if (player != null && slot != null)
      {
        ...
        Logger.LogProblems("[Room.RemovePlayer] Jogador '" + (object) player.player_id + "' '" + player.player_name + "'; OldSlot: " + (object) player._slotId + "; NewSlot: -1", "errorC");
        player._slotId = -1;
      ...
      lock (this._slots)
      {
        for (int index = 0; index < 16; ++index)
        {
          ...
          if (slot._playerId == 0L && slot.state == SLOT_STATE.EMPTY)
          {
            ...
            slot.state = SLOT_STATE.NORMAL;
            p._room = this;
            Logger.LogProblems("[Room.addPlayer] Jogador '" + (object) p.player_id + "' '" + p.player_name + "'; OldSlot: " + (object) p._slotId + "; NewSlot: " + (object) index, "errorC");
            p._slotId = index;
            slot._equip = p._equip;
    ...
    public int addPlayer(Account p, int teamIdx)
    {
      ...
      lock (this._slots)
      {
        for (int index1 = 0; index1 < teamArray.Length; ++index1)
        {
          ...
          SLOT slot = this._slots[index2];
          if (slot._playerId == 0L && slot.state == SLOT_STATE.EMPTY)
          {
            slot._playerId = p.player_id;
            slot.state = SLOT_STATE.NORMAL;
            p._room = this;
            Logger.LogProblems("[Room.addPlayer2] Jogador '" + (object) p.player_id + "' '" + p.player_name + "'; OldSlot: " + (object) p._slotId + "; NewSlot: " + (object) index1, "errorC");
            p._slotId = index2;
            slot._equip = p._equip;
            p._status.updateRoom((byte) this._roomId);
            AllUtils.syncPlayerToClanMembers(p);
    ...
    {
      this.round.StartJob(5250, (TimerCallback) (callbackState =>
      {
        if (this._state == RoomState.Battle)
        {
          using (BATTLE_TIMERSYNC_PAK battleTimersyncPak = new BATTLE_TIMERSYNC_PAK(this))
