  ...
  public static class AllUtils
  {
    ...
    private static void MissionCompleteBase(Room room, Account pR, SLOT slot, FragInfos kills, MISSION_TYPE autoComplete, int moreInfo)
    {
      try
      {
        ...
        if (missions == null)
        {
          Logger.error(DateTime.Now.ToString("HH:mm:ss") + ": Missions null1! by accountId: " + (object) slot._playerId);
        }
        else
        {
    ...
    private static void MissionCompleteBase(Room room, Account pR, SLOT slot, MISSION_TYPE autoComplete, int moreInfo)
    {
      try
      {
        ...
        if (missions == null)
        {
          Logger.error(DateTime.Now.ToString("HH:mm:ss") + ": Missions null2! by accountId: " + (object) slot._playerId);
        }
        else
        {
          int currentMissionId = missions.getCurrentMissionId();
    ...
    public static List<int> getDinossaurs(Room room, bool forceNewTRex, int forceRexIdx)
    {
      ...
      if (room.room_type == (byte) 7 || room.room_type == (byte) 12)
      {
        ...
        {
          Logger.warning("[" + DateTime.Now.ToString("HH:mm") + "] trex: " + (object) room.TRex);
          ...
          Logger.warning("[" + DateTime.Now.ToString("HH:mm") + "] forceRexIdx: " + (object) forceRexIdx + "; force: " + forceNewTRex.ToString() + "; teamIdx: " + (object) index + "; trex: " + (object) room.TRex);
        }
      }
      return intList;
    }
    ...
    public static void DiscountPlayerItems(SLOT slot, Account p)
    {
      ...
      int type = PlayerManager.CheckEquipedItems(p._equip, p._inventory._items, false);
      if (type <= 0)
        return;
    ...
    public static void TryBalancePlayer(Room room, Account player, bool inBattle, ref SLOT mySlot)
    {
      ...
      foreach (int index in team == 1 ? room.RED_TEAM : room.BLUE_TEAM)
      {
        ...
        if (slot3.state != SLOT_STATE.CLOSE && slot3._playerId == 0L)
        {
          slot2 = slot3;
          break;
        }
      ...
      if (slots.Count <= 0)
        return;
      Logger.LogProblems("[AllUtils.TryBalancePlayer] Jogador '" + (object) player.player_id + "' '" + player.player_name + "'; OldSlot: " + (object) player._slotId + "; NewSlot: " + (object) slot2._id, "errorC");
      player._slotId = slot1._id;
      mySlot = slot1;
      using (ROOM_CHANGE_SLOTS_PAK roomChangeSlotsPak = new ROOM_CHANGE_SLOTS_PAK(slots, room._leader, 1))
    ...
    public static int getBalanceTeamIdx(Room room, bool inBattle, int PlayerTeamIdx)
    {
      ...
      {
        SLOT slot = room._slots[index];
        if (slot.state == SLOT_STATE.NORMAL && !inBattle || slot.state >= SLOT_STATE.LOAD & inBattle)
        {
          if (slot._team == 0)
            ++num1;
          else
    ...
    public static void GetXmasReward(Account p)
    {
      ...
      p.SendPacket((SendPacket) new INVENTORY_ITEM_CREATE_PAK(1, p, new ItemsModel(702001024, 1, "Alcaçuz (Evento)", 1, 100U, 0L)));
    }
      ...
      if (room._channelType == 4)
      {
        foreach (ClanModel clanModel in (IEnumerable<ClanModel>) AllUtils.GetClanListMatchPlayers(room).Values)
        {
          if (clanModel.RedPlayers >= 1 && clanModel.BluePlayers >= 1)
          {
            ...
            Logger.warning("XP cancelado em clãfronto [Room: " + (object) room._roomId + "; Canal: " + (object) room._channelId + "; Clã: " + (object) clanModel.clanId + "]");
            return true;
          }
        }
      }
    ...
    public static void LogRoomBattleStart(Room room)
    {
      ...
      stringUtil.AppendLine("[Room] Tech id: " + (object) uniqueRoomId);
      ...
      stringUtil.AppendLine("[Room] Room type: " + (object) (RoomType) room.room_type);
      stringUtil.AppendLine("[Room] Room special: " + (object) (RoomSpecial) room.special);
      stringUtil.AppendLine("[Room] Room weapons: " + (object) (RoomWeaponsFlag) room.weaponsFlag);
      stringUtil.AppendLine("[Room] Map name: '" + room._mapName + "'");
      stringUtil.AppendLine("[Room] Actual players count: " + (object) room.getPlayingPlayers(2, true));
    ...
    public static void LogVotekickStart(Room room, Account p, SLOT slot)
    {
      ...
      uint startDate = room.StartDate;
      uint uniqueRoomId = room.UniqueRoomId;
      Logger.LogRoom("[Room] Votekick has been started by " + (object) slot._id + " (Id: " + (object) p.player_id + "; Nick: " + p.player_name + "), (To: " + (object) votekick.victimIdx + "; Motive: " + (object) (VoteKickMotive) votekick.motive + ")", startDate, uniqueRoomId);
    }
    ...
    public static void LogVotekickResult(Room room)
    {
      ...
      uint startDate = room.StartDate;
      uint uniqueRoomId = room.UniqueRoomId;
      Logger.LogRoom("[Room] Votekick result: (Allies: " + (object) votekick.allies + "; Enemys: " + (object) votekick.enemys + "); Total: (" + (object) votekick.kikar + "/" + (object) votekick.deixar + ")", startDate, uniqueRoomId);
    }
    ...
    public static void LogRoomRoundRestart(Room room)
    {
      uint startDate = room.StartDate;
      uint uniqueRoomId = room.UniqueRoomId;
      Logger.LogRoom("[Room] Round " + (object) room.rodada + " has been started.", startDate, uniqueRoomId);
    }
    ...
    public static void LogRoomResult(Room room)
    {
      ...
      stringUtil.AppendLine("[Room] Tech id: " + (object) uniqueRoomId);
      stringUtil.AppendLine("[Room] Used spawn count: " + (object) room.spawnsCount);
      stringUtil.AppendLine("[Room] End Players count: " + (object) room.getPlayingPlayers(2, true));
      stringUtil.AppendLine("[Room] ;------------------;");
      ...
      for (int index = 0; index < 16; ++index)
      {
        ...
        if (slot.state == SLOT_STATE.BATTLE && slot._playerId > 0L)
          stringUtil.AppendLine("[Room] Player (Id: " + (object) slot._playerId + "; Slot: " + (object) index + ") Exp: " + (object) slot.exp + "; Gp: " + (object) slot.gp + "; KD: " + (object) slot.allKills + "/" + (object) slot.allDeaths + "; Score: " + (object) slot.Score);
      }
      Logger.LogRoom(stringUtil.getString(), startDate, uniqueRoomId);
    }
  }
